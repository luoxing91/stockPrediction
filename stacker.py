...
# best is 133
model_ridge = lm.LogisticRegression(penalty='l2', dual=False,
                                    tol=0.0001, C=9081)
...
for i in range(10):
    ...
    indxs_to_fit = list(set(range(X.shape[0])) -
                        set(np.arange(i, X.shape[0], 10)))
    pred_ridge = pred_ridge+list(model_ridge
                                 .fit(X[indxs_to_fit, :], y[indxs_to_fit, :])
                                 .predict_proba(X[indxs, :])[:, 1])

    pred_randomforest += list(model_randomforest
                              .fit(X[indxs_to_fit, :], y[indxs_to_fit, :])
                              .predict_proba(X[indxs, :])[:, 1])
    new_Y = new_Y + list(y[indxs, :])

new_X = np.hstack((np.array(pred_ridge).reshape(len(pred_ridge), 1),
                   np.array(pred_randomforest)
                   .reshape(len(pred_randomforest), 1)))
print(new_X)
...
pred_randomforest_test = model_randomforest.fit(X, y).predict_proba(X_test)[:,
                                                                            1]

new_X_test = np.hstack((np.array(pred_ridge_test)
                        .reshape(len(pred_ridge_test), 1),
                        np.array(pred_randomforest_test)
                        .reshape(len(pred_randomforest_test), 1)))
...
pred_df = p.DataFrame(np.vstack((testindices, pred)).transpose(),
                      columns=["Id", "Prediction"])

pred_df.to_csv('./predictions/stacker/stacker' + strftime('%m-%d %X') + ".csv",
               index=False)
