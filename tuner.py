...
def numberOfOnes(actual):
    return len([0 for x in actual if x == 1])
...
def auc(actual, posterior):
    ...
    num_positive = numberOfOnes(actual)
...
X_test_stockindicators = np.vstack((np.identity(94)[:, range(93)]
                                    for i in range(25)))
...
X_stockindicators = np.vstack((np.identity(94)[:, range(93)]
...
if modelname == "sgd":
    ...
    models = [lm.SGDClassifier(loss="log", penalty="l2",
                               alpha=c, warm_start=False) for c in C]
...
for i, model in enumerate(models):
    cv_scores[i] = np.mean(cross_validation
                           .cross_val_score(model, X, y,
                                            cv=5, scoring=auc_scorer))
    print(" ({0}/{1}) C = {2}: CV = {3}"
          .format(i + 1, len(C), C[i], cv_scores[i]))
...
best_C = C[best]
print("BEST {}: {}".format(best_C, best_cv))
...
pred_df = p.DataFrame(np.vstack((testindices, pred))
                      .transpose(), columns=["Id", "Prediction"])
filename = './predictions/' + modelname + '/' + modelname + ' ' + \
           strftime('%m-%d %X') + ' C-' + str(round(best_C, 4)) + \
           ' CV-'+str(round(best_cv, 4))+'.csv'
